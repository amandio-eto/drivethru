<?php
   ...
   $kata = mysqli_escape_string($conn,$_GET['kata']);
	$q = mysqli_query($conn,"select * from produk where nama like '%$kata%' order by id");
	if (mysqli_num_rows($q)>0) 
	{
		...
		?>
		   <a href="javascript:void(0)" onclick="tambah_makanan(<?php echo $id?>)" style="background:#fff;">
			   <div class="text-center" style="width:185px;background:#fff;padding:5px 8px;margin:4px 4px;border:1px solid #ddd;float:left">
